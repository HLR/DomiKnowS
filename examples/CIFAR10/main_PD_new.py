...
def model_declaration():
    ...
    image['pixels'] = ReaderSensor(keyword='pixels', device="cuda:0")
    image[airplane] = ReaderSensor(keyword='airplane',label=True, device="cuda:0")
    image[dog] = ReaderSensor(keyword='dog',label=True, device="cuda:0")
    image[truck] = ReaderSensor(keyword='truck',label=True, device="cuda:0")
    image[automobile] = ReaderSensor(keyword='automobile',label=True, device="cuda:0")
    image[bird] = ReaderSensor(keyword='bird',label=True, device="cuda:0")
    image[cat] = ReaderSensor(keyword='cat',label=True, device="cuda:0")
    image[deer] = ReaderSensor(keyword='deer',label=True, device="cuda:0")
    image[frog] = ReaderSensor(keyword='frog',label=True, device="cuda:0")
    image[horse] = ReaderSensor(keyword='horse',label=True, device="cuda:0")
    image[ship] = ReaderSensor(keyword='ship',label=True, device="cuda:0")

    image['emb'] = ModuleLearner('pixels', module=ImageNetwork(), device="cuda:0")
    image[airplane] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[dog] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[truck] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[automobile] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[bird] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[cat] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[deer] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[frog] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[horse] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
    image[ship] = ModuleLearner('emb', module=nn.Linear(16 * 5 * 5, 2), device="cuda:0")
...
def main():
    ...
    program.train(training_set=train_ds, valid_set=val_ds, test_set=testset, device="cuda:0", train_epoch_num=30, Optim=lambda param: torch.optim.SGD(param, lr=.001))
