...
def main():
    ...
    parser.add_argument('--samplenum', dest='samplenum', default=1000,help='number of samples to choose from the dataset',type=int)
    ...
    train_reader,test_reader=create_readers(train_num=args.samplenum,test_num=args.samplenum)

    program.train(train_reader, train_epoch_num=args.epochs, Optim=lambda param: torch.optim.SGD(param, lr=args.learning_rate),device=device)
    guessed_tag = {
        "local/softmax": [],
        "ILP": []
    }
