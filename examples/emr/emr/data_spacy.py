...
def remove_relations(start, length, relations):
    ...
    for j in reversed(range(len(relations))): # descending order to avoid index change
...
def add_tokens_relations(tokens, relations):
    for j, relation in enumerate(relations):
        ...
        relations[j] = (relation_type, (src_index, src_token), (dst_index, dst_token))
    return relations
...
def reprocess(sentence, relations, keep_entity=False, first=True):
    ...
    for i, new_token in enumerate(new_tokens):
        ...
        if i > 0 and new_token in MERGE and new_tokens[i-1] == new_token:
            if first:
                new_tokens[i-1] = new_token * 2
                new_tokens[i] = ''
            else:
                new_tokens[i-1] = ''
                new_tokens[i] = new_token * 2
...
class Conll04SpaCyReader(SensableReader):
    ...
    def raw_read(self, file_path):
        ...
        for sentence, relations in zip(sentences, relation_lists):
            ...
            # TODO use spacy here
            #tokens, labels, relations = spacy_process(tokens, labels, relations)
            yield new_sentence, new_labels, new_relations
