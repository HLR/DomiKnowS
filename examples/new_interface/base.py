...
class ACEGraph(PytorchSolverGraph, metaclass=WrapperMetaClass):
    ...
    def weights(self, info, truth):
        weights = []
        defaults = [pow(0.96312867737412, 4), pow(0.9894575918349645, 4), pow(0.8696203721105056, 4), pow(0.9933500065054284, 4), pow(0.9916224538475995, 4), pow(0.9943908750524049, 4), pow(0.9453833142989316, 4)]
            ...
            for j in tqdm(range(len(paths)), "READER : "):
                while True:
                    try:
                        ...
                        result = self.solver.inferILPConstrains(context=context)
                        print(result)
                        total_loss = 0
                        weights = self.weights(info=info, truth=truth)
                        loss_fn = []
