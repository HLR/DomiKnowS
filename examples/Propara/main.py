...
def model_declaration():
    ...
    step[procedure_contain_step.forward, "text"] = JoinEdgeReaderSensor(
        procedure["id"],
        keyword="steps",
        relation=procedure_contain_step,
        mode="forward",
    )
    # word[step_contains_word, 'raw'] = ReaderSensor(keyword='words')
    entity["raw"] = ReaderSensor(keyword="entities")
    entity_step[entity_of_step.forward, step_of_entity.forward] = JoinReaderSensor(
        entity["raw"],
        step["text"],
        step[procedure_contain_step.forward],
        keyword="entity_step",
    )
    ...
    entity_step[unknown_loc] = ReaderSensor(keyword="known")
    entity_step[known_loc] = ReaderSensor(keyword="unkown")
    action[action_arg1.forward, action_arg2.forward] = JoinReaderSensor(
        entity_step[entity_of_step.forward],
        entity_step[step_of_entity.forward],
        keyword="action",
    )
    action[create] = ReaderSensor(
        action_arg1.forward, action_arg2.forward, keyword="create"
    )
    action[destroy] = ReaderSensor(
        action_arg1.forward, action_arg2.forward, keyword="destroy"
    )
    action[other] = ReaderSensor(
        action_arg1.forward, action_arg2.forward, keyword="other"
    )
    action[create] = ReaderSensor(
        action_arg1.forward, action_arg2.forward, keyword="create"
    )
...
def main():
    ...
    for datanode in lbp.populate(dataset, device="cpu"):
        ...
        datanode.inferILPResults(
            fun=lambda val: torch.tensor(val)
            .softmax(dim=-1)
            .detach()
            .cpu()
            .numpy()
