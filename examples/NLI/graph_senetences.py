...
with Graph('Useful_arg') as graph:
    ...
    # Ent(X1, X2) => !CON(X2, X1)
    ifL(andL(entailment('x'), symmetric('s', path=('x', symmetric))), notL(contradiction(path=('s', s_sent2))))
    # Neu(X1, X2) => !CON(X2, X1)
    ifL(andL(neutral('x'), symmetric('s', path=('x', symmetric))), notL(contradiction(path=('s', s_sent2))))
    # CON(X1, X2) => CON(X2, X1)
    ifL(andL(contradiction('x'), symmetric('s', path=('x', symmetric))), contradiction(path=('s', s_sent2)))
