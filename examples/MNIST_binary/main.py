...
parser.add_argument('--lr', dest='learning_rate', default=1e-4, help='learning rate of the adam optimiser',type=float)
...
parser.add_argument('--samplenum', dest='samplenum', default=5, help='number of samples to train the model on',type=int)
...
mnist_testset_reader=create_readers(mnist_testset,len(mnist_testset)//args.batch_size+1,args.batch_size)
...
for number, i in enumerate(labels):
    image[i] = ModuleLearner('pixels', module=MNISTCNN((1, 28, 28), 2))
...
program.train(mnist_trainset_reader, train_epoch_num=args.cur_epoch, Optim=lambda param: torch.optim.SGD(param, lr=args.learning_rate),device=device)
program.train(mnist_testset_reader,device=device)
