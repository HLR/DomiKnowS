...
class InteractionReader(RegrReader):
    ...
    def parse_file(self):
        ...
        for l in content[:size]:
            ...
            for s in substring[0].strip().split():
                situation.append(s)
            
            utterance = []
            
            for w in substring[1].strip().split():
                
                # if w not in ('the','of','to'):
                #     utterance.append(w)
                    
                utterance.append(w)
                
            # Utterances must have a length of max_length
            # if len(utterance) < 12:
            #     diff = 12 - len(utterance)
                
            #     for k in range(diff):
            #         utterance.append("<eos>")
            
            # if len(situation) != len(utterance):
            #     continue
