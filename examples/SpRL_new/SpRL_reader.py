...
class SpRLReader(SensableReader):
    ...
    def negative_relation_generation(self, phrase_relation_list):
        gold_relation_region=0
        gold_relation_direction=0
        gold_relation_distance=0
        negative_relation=0
                ...
                if indice in positive_list:
                    ...
                    for positive_text in positive_text_list:
                        ...
                        if label_set == positive_set:
                            ...
                            break
                        elif len(label_set.intersection(positive_set))==1 or len(label_set.intersection(positive_set))==0:
                            break
                        else:
    ...
    def negative_entity_generation(self, phraselist):
        ...
        for phrase in phraselist:
            try:
                ...
                chunklist = DataFeature(phrase['TEXT'],"").getChunks()
                ...
                for chunk in chunklist:
                    ...
                    if chunk.text in tag1list or chunk.text  in tag2list or chunk.text  in tag3list:
                        continue
                    elif DataFeature("",chunk.text ).getHeadword() in tag1_headword and DataFeature("",chunk.text ).getPhrasepos() != "VERB":

                        tag1list.append(chunk.text )

                    elif DataFeature("",chunk.text ).getHeadword() in tag2_headword and DataFeature("",chunk.text ).getPhrasepos() != "VERB":

                        tag2list.append(chunk.text )
                    elif DataFeature("",chunk.text ).getHeadword() in tag3_headword and DataFeature("",chunk.text ).getPhrasepos() != "VERB":

                        tag3list.append(chunk.text )

                    else:
                        labelnone.append(chunk.text )
