...
class lcLossBooleanMethods(constraintsProcessor):
    ...
    def ifVar(self, _, var1, var2, onlyConstrains = False):
        ...
        # Element-wise: scalars, vectors and [rules, groundings] matrices.
    ...
    def ifVarBatched(self, _, var1, var2, onlyConstrains=False):
        ...
        numerically equivalent to one call to :meth:`ifVar`, which is
        element-wise.  Keeping this primitive beside ``ifVar`` prevents the
        compiled graph fast path from maintaining a second set of t-norm
        equations.
        """
        var1, var2 = self._fixVar((var1, var2))
        ...
        if self.tnorm in ('L', 'G', 'P'):
            return self.ifVar(_, var1, var2, onlyConstrains=onlyConstrains)
        else:
            raise ValueError(
