...
class ILPTransformerConfig(PretrainedConfig):
    r"""
...
class ILPTransformerEmbeddings(nn.Module):
...
class ILPTransformerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in ILPTransformerModel forward() function)
...
class ILPTransformerSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ILPTransformerAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = ILPTransformerSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = ILPTransformerSelfOutput(config)
...
class ILPTransformerIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class ILPTransformerOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.intermediate_size, 2)
        # self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class ILPTransformerLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = ILPTransformerAttention(config)
        self.is_decoder = False
        self.add_cross_attention = config.add_cross_attention
        self.intermediate = ILPTransformerIntermediate(config)
        self.output = ILPTransformerOutput(config)
