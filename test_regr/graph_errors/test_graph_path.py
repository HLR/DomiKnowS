...
def test_setup_graph_exception():
    try:
        ...
        sanitized_pattern = re.sub(r'[^\x20-\x7E]', '', "The Path rel_pair_entity1 from the variable x, defined in LC_employment is not valid"
                                           " and the type of x is a relation, there is no relationship defined between named_entity and relation." 
                                           " The used variable rel_pair_entity1 is a relationship defined between a pair and a named_entity,"
                                           " which is not correctly used here.").replace(" ", "")
