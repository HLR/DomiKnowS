...
class InternVLShared(torch.nn.Module):
    ...
    def __init__(self, model_path="OpenGVLab/InternVL3_5-8B", device="cuda", relation=1, attr="no name", *args, **kwargs):
        ...
        if InternVLShared.model is None:
            # Assuming InternVL class is available in your scope
    ...
    def forward(self, image, image_filename, bounding_boxes):
        ...
        if self.relation == 2:
            ...
            for box in bounding_boxes:
                ...
                ImageDraw.Draw(img_copy).rectangle(box.tolist(), outline="red", width=3)
                images.append(img_copy)
                questions.append(f"Is the object in the red bounding box {self.attr}? answer with only Yes or No.")
        ...
        # --- Debug Visualization ---
        if False:
            print(f"\n[DEBUG] Visualizing {len(images)} pairs:")
            for i, (img, q, ans) in enumerate(zip(images, questions, results)):
                ...
                print(f"Question: {q}")
                print(f"Model Output: {ans}") # This prints the score/answer tensor
                print("-" * 50)
