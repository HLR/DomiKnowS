...
\tobject = Concept(name='object')
\timage_object_contains, = image.contains(object)\n\n"""

    for attr, values in g_attribute_concepts.items():
        for val in values:
            prop_name = f"is_{val}"
            graph_text += f"\t{prop_name} = object(name='{prop_name}')\n"
        graph_text+='\n'
    ...
    return executions,local_vars["graph"],local_vars["image"],local_vars["object"],local_vars["image_object_contains"],{attr_name:local_vars[attr_name] for attr_name in local_vars.keys() if "is_" in attr_name}