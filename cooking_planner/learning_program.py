"""Graph-HMM learning helpers for the cooking planner demo."""
...
from domiknows.generation.graph_hmm import DomiKnowSAwareHMM, GraphHMMGenerationHead
from domiknows.generation.planning import PlanningBundle, encode_plan, planning_hmm_masks_from_graph


@dataclass
class CookingHMMArtifacts:
...
def fit_graph_hmm(
    ...
    *,
    ...
    sequences = list(bundle.reference_plans.values())
    hmm = DomiKnowSAwareHMM(
        graph=bundle.graph,
    ...
    hmm.fit(sequences, max_iter=max_iter)
    return CookingHMMArtifacts(hmm=hmm, transition_mask=transition_mask, emission_mask=emission_mask)
...
def supervised_head_loss(head: GraphHMMGenerationHead, bundle: PlanningBundle, plan=None) -> torch.Tensor:
    """Return a simple teacher-forced NLL for one graph-declared plan."""
