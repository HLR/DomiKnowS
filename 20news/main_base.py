...
def main(device):
    ...
    rep_model_path = f"{prefix}rep_model_base.pt"
    ...
    level1_classification_path = f"{prefix}level1_classification_base.pt"
    level1_classification.load_state_dict(torch.load(level1_classification_path, map_location=device))
    level2_classification_path = f"{prefix}level2_classification_base.pt"
    level2_classification.load_state_dict(torch.load(level2_classification_path, map_location=device))
    level3_classification_path = f"{prefix}level3_classification_base.pt"
...
if __name__ == '__main__':
    ...
    from reader_base import collate_label_set
    from graph import *
    ...
    prefix = "Tasks/20news/"
    with open(f"{prefix}logger_base.json", "w") as file:
        json.dump(f1_res, file, indent=4)

    file2 = open(f"{prefix}logger_manual_base.txt", "w")
    for _c in corrects:
        if totals[_c] != 0:
