"""Compare the three compact-label learners under the nested-constraint DFA.

Trains each of ``discrete-hmm``, ``graph-hmm``, and ``energy`` on the same
generator stream and evaluates them on each demo prompt (``with_A`` /
``with_C``).  Reports the side-by-side metrics that matter for picking a
learner:

* training time and inference time
* trainable parameter count
* DFA acceptance of the greedy output under the three nested LCs
* prompt-conditioning: are the two prompts producing distinct outputs?
...
def _evaluate_learner(artifacts, report: LearnerReport, *, prompts: Iterable[str]) -> LearnerReport:
...
def _print_header(args) -> None:
    print("Performance comparison across compact-label learners -- nested-constraint DFA")
    print("  Constraints: EOS-closure (path), atMostAL(B, 1), not(existsA and existsC), existsA or existsC, forbidden-D")
    print(
...
def _print_per_learner_summary(report: LearnerReport) -> None:
    ...
    print(f"  trainable params : {report.trainable_param_count} across {report.trainable_tensor_count} tensors")
    print(f"  training time    : {_format_seconds(report.train_seconds)} (over the configured stream batches)")
    for prompt_name, result in report.prompt_results.items():
        ...
        print(
            f"  prompt {prompt_name:>7} ({info['text']}): "
            f"{_format_symbols(result.symbols)}  "
            f"[score {result.log_score:7.3f}, dfa {marker}, infer {_format_seconds(result.inference_seconds)}]"
        )
