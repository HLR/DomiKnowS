...
class GurobiSolver(Solver):
    ...
    def solve_legacy(self, data, *predicates_list):
        ...
        if self.self_relation:
            gen = lambda enum_data, order: product(enum_data, repeat=order)
        else:
            gen = lambda enum_data, order: permutations(enum_data, r=order)
        for order, predicates in enumerate(predicates_list, 1):
            for concept in predicates:
                #assert concept not in candidates
                # last one change first (c-order)
                # abc rep=3 -> aaa, aab, aac, aba, abb, abc, ...
                candidates[concept] = tuple(gen(enumerate(data), order))
...
#                         print(constr)
        ...
        for predicates in predicates_list:
            ...
            for concept, predicate in predicates.items():
                predicates_result[concept] = np.zeros((length,) * len(candidates[concept][0]))
                for obj in candidates[concept]:
