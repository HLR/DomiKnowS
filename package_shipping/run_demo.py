"""Run the declarative package shipping graph-HMM demo."""
...
try:
    ...
    from .planner_agent import MockPackageShippingPlannerAgent
except ImportError:
    from graph import build_graph
    from learning_program import build_graph_hmm_head, fit_graph_hmm, run_one_head_step
    from planner_agent import MockPackageShippingPlannerAgent
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--task", choices=("ship_book", "ship_fragile_vase", "return_item"), default="ship_book")
    ...
    graph, _parts = build_graph(args.task)
    bundle = planning_bundle_from_graph(graph, selected_task=args.task, **PLANNING_KWARGS)
    ...
    print("Package shipping path: declarative DomiKnowS graph -> planning DFA + graph-HMM")
    print(f"Task: {bundle.selected_task}")
    ...
    planner = MockPackageShippingPlannerAgent(seed=args.seed)
    scored = []
    print("\nCandidate plans:")
